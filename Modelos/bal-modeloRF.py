...
def entrenar_modelo(X_train, y_train):
    ...
    modelo = RandomForestClassifier(criterion='entropy', max_depth=7, min_samples_leaf=1, min_samples_split=4, n_estimators=20, random_state=42)
...
def probar_modelo(modelo, X_test, y_test):
    ...
    plt.savefig('../Imagenes/Balanceado/RF/matriz_conf_RF.png')
...
def trazar_ROC(model, X_test, y_test):
    ...
    plt.title('ROC de RF Balanceado')
    plt.legend(loc="lower right", fontsize='small', ncol=2, title='Clases')       
    plt.savefig('../Imagenes/Balanceado/RF/ROC_RF.png')
...
if __name__ == '__main__':
    ...
    guardar_modelo(modelo, '../Datos/Balanceado/modeloRF.pkl')
    tf = time.time()
